...
class MenuOpcoesAlunos: NSObject {
    ...
    func configuraMenuDeOpcoesDoAluno(navegation: UINavigationController, alunoSelecionado: Aluno) -> UIAlertController {
        let menu = UIAlertController(title: "Atencao", message: "escolha uma das opcoes abaixo", preferredStyle: .actionSheet)
        guard let viewController = navegation.viewControllers.last else { return menu }
        ...
        let waze = UIAlertAction(title: "localizar no waze", style: .default) { (acao) in
            Localizacao().localizaAlunoNoWaze(alunoSelecionado: alunoSelecionado)
        }
        menu.addAction(waze)
        ...
        let mapa = UIAlertAction(title: "localizar no mapa", style: .default) { (acao) in
            ...
            navegation.pushViewController(mapa, animated: true)
        }
        menu.addAction(mapa)
